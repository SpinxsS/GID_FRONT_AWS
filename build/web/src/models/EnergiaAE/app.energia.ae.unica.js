...
$("#btnReport").on('click', function (){
    ...
    if (empty(fhi,fhf,id)) {
        ...
        $("#loadTableEnergy").html("");
        $("#loadTableEnergy").html("<h3 class='text-center'><img src='../../template/img/load.gif' width='100' height='80'></h3>");
        
        $.ajax({
            url:'http://'+ readConfig() + '/consultas/energiaAE/'+ id + '/' + convertFh(fhi)+ '/' + convertFh(fhf),
            type: 'GET',
            dataType: "json"
...
function valueReport(data){
    var response = "";
    $.each(data, function (key, val){
       response += tableReport(val.CODIGO_SIC, val.FECHA,val.A00, val.A01, val.A02, val.A03, val.A04, val.A05, val.A06, val.A07, val.A08, val.A09, val.A10, val.A11, val.A12, val.A13, val.A14, val.A15, val.A16, val.A17, val.A18, val.A19, val.A20, val.A21, val.A22, val.A23);
    });
